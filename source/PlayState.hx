...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		#if windows
		executeModchart = FileSystem.exists(Paths.lua("data/" + songLowercase  + "/modchart"));
		#end
		#if !cpp
		...
		kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + (CoolUtil.difficultyString()) + (Main.watermarks ? " - KE " + MainMenuState.kadeEngineVer : ""), 16);
		...
		for (folder in foldersToCheck)
		{
			if(FileSystem.exists(folder))
			{
				for (file in FileSystem.readDirectory(folder))
				{
					if(file.endsWith('.lua') && !filesPushed.contains(file) && !file.startsWith('modchart'))
					{
						luaArray.push(new FunkinLua(folder + file));
						filesPushed.push(file);
					}
	...
	public function changeCharacter(value1:String, value2:String) {
		var charType:Int = 0;
		switch(value1) {
			case 'gf' | 'girlfriend':
				charType = 2;
	...
	public function startCountdown():Void
	{
		...
		for (i in 0...cpuStrums.length) {
			...
			//if(ClientPrefs.middleScroll) cpuStrums.members[i].visible = false;
		}
		...
		if (executeModchart)
		{
			luaModchart = new ModchartState(Paths.lua("data/" + songLowercase  + "/modchart"));
	...
	private function generateStaticArrows(player:Int):Void
	{
       ...
       if(PlayState.PlayState.isPixelStage) {
					babyArrow.loadGraphic(Paths.image('pixelUI/' + texture), true, 17, 17);
					...
					switch (Math.abs(i))
					{
						...
						babyArrow.frames = Paths.getSparrowAtlas(texture);
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null)
		{
			...
			if (camFollow.x != dad.getMidpoint().x + 150 && !PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection && !isCameraOnForcedPos)
			{
				var offsetX = 0;
				var offsetY = 0;
				#if windows
			...
			if (camFollow.x != gf.getMidpoint().x && PlayState.SONG.notes[Std.int(curStep / 16)].gfSection && !isCameraOnForcedPos)
			{
			...
			if (PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection && camFollow.x != boyfriend.getMidpoint().x - 100 && !isCameraOnForcedPos)
			{
				var offsetX = 0;
				var offsetY = 0;
				#if windows
				...
				// i tried doing psych method but it wont fucking work so i guess we got this for now
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					FlxG.save.data.weekUnlocked = StoryMenuState.weekCompleted;
					FlxG.save.flush();
				}
				else
				{
	...
	function noteMiss(direction:Int = 1, daNote:Note):Void
	{
		if (!boyfriend.stunned)
		{
			...
			callOnLuas('noteMiss', [notes.members.indexOf(daNote), daNote.noteData, daNote.isSustainNote]);
